...
void test_op_latency(Client & client, const char * op, int num_op){
    ...
    for (int i = 0; i < client.num_local_operations_; i ++) {
        ...
        case KV_REQ_SEARCH:
            ...
            search_addr = client.race_search(ctx);
        ...
        case KV_REQ_INSERT:
            clock_gettime(CLOCK_REALTIME, &st);
            ret = client.race_insert(ctx);
            clock_gettime(CLOCK_REALTIME, &et);
            client.kv_insert_buffer(ctx);
    ...
    */
    ...
    char file_name[128];
    sprintf(file_name, "results/op-%s_k-%d_m-%d_RACE_nolog.txt", op, client.k, client.m);
