...
WorkloadFileName * get_workload_fname(int cluster_id, int thread_id) {
    ...
    sprintf(workload_fname->load_fname, "cluster/cluster%d_load", cluster_id);
    sprintf(workload_fname->trans_fname, "cluster/cluster%d_test%d", cluster_id, thread_id);
...
int load_workload(Client & client, WorkloadFileName * workload_fnames, int st, int ed) {
    ...
    ret = client.load_kv_req_file_twitter(workload_fnames->load_fname, st, ed);
    assert(ret == 0);
    int num_coro = 1;
...
int test_workload(Client & client, WorkloadFileName * workload_fnames, RunClientArgs * args) {
    ...
    ret = client.load_kv_req_file_twitter(workload_fnames->trans_fname, 0, 100000);
    ...
    client.print_count_used_slot();
    return 0;
}

void * run_client_twitter(void * _args) {
    RunClientArgs * args = (RunClientArgs *)_args;
    WorkloadFileName * workload_fnames = get_workload_fname(args->cluster_id, args->thread_id + args->client_id);
    int ret = 0;
    GlobalConfig config;
    ...
    if (args->thread_id == 0 && pre_client_id == 0) {
        ret = load_workload(client, workload_fnames, 0, -1);
    } 
    ret = test_workload(client, workload_fnames, args);
