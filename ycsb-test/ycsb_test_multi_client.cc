...
int main(int argc, char ** argv) {
    ...
    for (int i = 0; i < num_clients; i ++) {
        ...
        pthread_t tid;
        pthread_create(&tid, NULL, run_client_tpt, &client_args_list[i]);
        tid_list[i] = tid;
    }
    ...
    int tpt = (total_tpt - total_failed) / config.workload_run_time;
    printf("total: %d ops\n", total_tpt);
    printf("failed: %d ops\n", total_failed);
    printf("tpt: %d ops/s\n", tpt);
    ...
    char file_name[128];
    FILE *of;
    sprintf(file_name, "./results/MetaEC-clients_%d-thread_%d-%s.txt", 
        config.all_clients, config.server_id, argv[2]);
    printf("filename:%s\n", file_name);
