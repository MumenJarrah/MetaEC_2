...
void ClientMM::check_subblock(){
    for(int i = 0;i < num_memory_;i ++){
        cout << "size:" << subblock_free_queue_[i].size() << endl;
    }
}

int32_t ClientMM::dyn_get_new_block_from_server(UDPNetworkManager * nm, uint64_t server_id, int is_enc) {
...
void ClientMM::mm_alloc(size_t size, UDPNetworkManager * nm, uint64_t server_id,  __OUT ClientMMAllocCtx * ctx) {
...
void ClientMM::mm_alloc_encoding(size_t size, UDPNetworkManager * nm, uint64_t server_id,  __OUT ClientMMAllocCtx * ctx) {
...
void ClientMM::mm_alloc_small_subtable(UDPNetworkManager * nm, __OUT ClientMMAllocSubtableCtx * ctx, uint8_t server_id) {
    struct MrInfo mr_info_list;
...
void ClientMM::mm_alloc_big_subtable(UDPNetworkManager * nm, __OUT ClientMMAllocSubtableCtx * ctx, uint8_t server_id) {
    struct MrInfo mr_info_list;
...
void ClientMM::mm_alloc_log(UDPNetworkManager * nm, __OUT ClientMMAllocLogCtx * ctx) {
...
int ClientMM::alloc_from_sid(uint32_t server_id, UDPNetworkManager * nm, int alloc_type, __OUT struct MrInfo * mr_info) {
...
int ClientMM::local_reg_blocks(const struct MrInfo * mr_info_list, const uint8_t * server_id_list, int is_enc) {  
...
int ClientMM::reg_new_space(const struct MrInfo * mr_info_list, const uint8_t * server_id_list, UDPNetworkManager * nm, int alloc_type) {
...
int ClientMM::init_reg_space(struct MrInfo mr_info_list[], uint8_t server_id_list[], UDPNetworkManager * nm, int alloc_type) {
    uint8_t server_id;
...
int ClientMM::dyn_reg_new_space(const struct MrInfo * mr_info_list, const uint8_t * server_id_list, UDPNetworkManager * nm, int alloc_type, int is_enc) {
    // locally register blocks
    if (alloc_type == TYPE_KVBLOCK) {
...
void ClientMM::print_leave_block(){
