...
void * serverfr_main(void * server_main_args) {
    ServerFRMainArgs * args = (ServerFRMainArgs *)server_main_args;
    ServerFR * server_instance = args->server;
    int ret = stick_this_thread_to_core(args->core_id);
    return server_instance->thread_main();   
}

ServerFR::ServerFR(const struct GlobalConfig * conf) {
    ...
    mm_ = new ServerFRMM(conf->server_base_addr, conf->server_data_len, conf->allocate_size, &ib_info, conf);
}
...
int ServerFR::server_on_connect(const struct KVMsg * request, 
...
// for alloc chunk(for client to allocate blocks to write kv chunk and parity chunk)
int ServerFR::server_on_alloc(const struct KVMsg * request, struct sockaddr_in * src_addr, 
        socklen_t src_addr_len) {
...
// for alloc race bucket for client to cas slot
int ServerFR::server_on_alloc_small_subtable(const struct KVMsg * request, struct sockaddr_in * src_addr,
        socklen_t src_addr_len) {
...
// for alloc race bucket for client to cas slot
int ServerFR::server_on_alloc_big_subtable(const struct KVMsg * request, struct sockaddr_in * src_addr,
        socklen_t src_addr_len) {
